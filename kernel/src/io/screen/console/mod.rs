...
const BUFFER_HEIGHT: usize = 48; //25;
...
impl Console {
        ...
        if self.row >= BUFFER_HEIGHT-1 {
            ...
            for row in 1..BUFFER_HEIGHT {
                for col in 0..BUFFER_WIDTH {
                    ...
                    //     self.write_char(col, row - 1, character);
                    // }
                }
            }
            self.row -= 1;
    ...
    fn write_char(&mut self, x: usize, y: usize, screen_char: ScreenChar) {
        ...
        let x_pos = x * width;
        let y_pos = y * height;
        screen::char_bitmap(x_pos, y_pos, 2, screen_char.fg, screen_char.bg, screen_char.ascii_character as char);
    }
}
...
macro_rules! print {
    ($($arg:tt)*) => {
        $crate::io::screen::console::_print(format_args!($($arg)*))
    };
}
